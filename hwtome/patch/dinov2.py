...
class ToMeDinoV2Block(Block):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        def attn_residual_func(x: torch.Tensor) -> Tuple:
            attention, metric = self.attn(self.norm1(x))
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            # Raise error 
            raise AssertionError("ToMe wrapping does not support the drop_add_residaul_stochastic_depth flag during training")
...
class ToMeDinoV2NestedTensorBlock(ToMeDinoV2Block):
    def forward(self, x_or_x_list):
        if isinstance(x_or_x_list, torch.Tensor):
            ...
            raise AssertionError("ToMe wrapping does not support list of tensors / nested tensors")
            if not XFORMERS_AVAILABLE:
                raise AssertionError("xFormers is required for using nested tensors")
...
def apply_patch(
    ...
    for module in model.backbone.modules():
        ...
        if isinstance(module, MemEffAttention):
            if model._tome_info["prop_attn"]:
                print("tome/patch/dinov2.py: Proportional attention will not be applied to MemEfficientAttention")
